...
開いているソースコードをクリップボードにコピーし、AtCoder の提出ページをブラウザで開く。
コンパイル言語の場合は事前にコンパイルチェックを行い、エラーがあれば提出を中止する。
ユーザは提出ページ上で `Ctrl+V` (macOS では `Cmd+V`) でコードを貼り付けて提出する。
...
各問題ディレクトリに移動したあと `am s` コマンドにファイル名を引数として指定することで、ソースコードをクリップボードにコピーし提出ページをブラウザで開く。
例: `am s Main.java` のように実行。
コンパイル言語の場合は事前にコンパイルチェックを行う。
ブラウザで開いた提出ページ上で `Ctrl+V` (macOS では `Cmd+V`) でコードを貼り付けて提出する。
...
`am s` コマンドは、ソースコードをクリップボードにコピーし、AtCoder の提出ページをブラウザで開く。
クリップボードへのコピーは OSC 52 エスケープシーケンスを利用するため、ターミナルが OSC 52 に対応している必要がある。
VS Code の統合ターミナルでは `devcontainer.json` で `terminal.integrated.osc52` を設定済み。
提出ページは問題が自動選択された状態で開くため、コードを貼り付けて言語を選択し提出するだけでよい。
